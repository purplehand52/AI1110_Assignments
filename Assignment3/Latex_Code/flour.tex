...
%Random Variable-------------------------------------------
We will define random variable $X$ such that $X \in \mathbb{R}$ and $X$ represents the weight of the bag.

%Probability-----------------------------------------------
We are required to find the probability when random variable exercise $X > 5$.
Note that the number of bags with $X > 5$ is given by
\begin{align}
  n(X > 5) = 7
...
Hence the probability where $X > 5$ from
\eqref{eq:FavOutcomes}
and
\eqref{eq:TotOutcomes}
is given by
\begin{align}
  P(X > 5) = \frac{n(X > 5)}{n(S)}
  \\
  =\frac{7}{11}
  \\
  =\text{\fbox{0.636}}
